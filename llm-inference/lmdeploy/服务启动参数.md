> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[llm-inference/README]] [[llm-inference/vllm/README]] [[llm-optimizer/kv-cache]] [[llm-compression/quantization/量化基础]]
> 📖 导读：本文逐项罗列 LMDeploy `lmdeploy serve api_server` 服务启动的全部命令行参数（后端选择、TP 并行、KV Cache 配置、量化策略、前缀缓存等），适合部署调优、排查启动选项含义时当作速查手册。
