> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[llm-train/peft/PEFT-API]] [[llm-compression/quantization/量化基础]] [[llm-train/README]]
> 📖 导读：本文是参数高效微调系列第五篇，深入讲解 LoRA（低秩分解模拟权重增量）、AdaLoRA（按重要性动态分配秩）与 QLoRA（4bit NF4 量化 + LoRA），适合想搞懂主流 PEFT 方法原理与显存优化的读者精读。
