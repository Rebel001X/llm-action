> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[昇腾NPU]] [[AI芯片软件生态]] [[硬件对比]] [[megatron-lm/README]]
> 📖 导读：本文记录在华为昇腾 NPU 上基于 MindSpore + MindFormers 框架跑大模型（含 Docker 镜像、CANN 环境、训练/推理脚本）的实操步骤；适合做国产 AI 芯片迁移、昇腾环境搭建与大模型适配时查阅。
