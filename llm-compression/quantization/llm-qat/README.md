...
> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[llm-compression/quantization/量化基础]] [[llm-compression/README]] [[llm-train/peft/PEFT-API]] [[llm-compression/quantization/GPTQ]]
> 📖 导读：本文是 LLM-QAT（量化感知训练）的环境搭建与复现实操记录，涵盖 apex 编译、虚拟环境配置与依赖安装；适合在量化训练（而非训练后量化）时复现 Meta LLM-QAT 流程的工程同学查阅。
