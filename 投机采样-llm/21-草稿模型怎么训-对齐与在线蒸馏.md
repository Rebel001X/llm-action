...
固定基准 $(\alpha{=}0.75,\,c{=}0.02)$ 的理想加速比 3.1989×（口径：理想模型 $E[\tau]/(\gamma c+1)$，**batch size = 1 且落在 memory-bound 区**，假设"验证免费"，与 §6.1 同口径；是上界不是实测），二分求"让收益打平"的最大 $c$：

（**第三列是"草稿成本 $c$ 相对基准 $c{=}0.02$ 的倍数"，主语是成本不是速度** —— 全表的加速比恒等于最后一列的 3.1989，正是"等收益线"的定义。）
...
- **Aurora: Adaptive Speculative Training for LLMs** —
  <https://arxiv.org/abs/2602.06932>（v1 2026-02-06，会议**未查证** 〔2026-08-22 核：arXiv abs 页 Comments 与 Journal-ref **均为空**，Together AI 博客也**未提任何会议**，故 ICML 2026 降级为「未查证」；论文与博客均未列作者机构，「Together AI 主导」同样**未经证实**，只能说博客发布于 Together AI 站点〕；
  项目页 <https://aurora-spec-ai.github.io/>，博客 <https://www.together.ai/blog/aurora>，代码 github.com/togethercomputer/aurora）。
