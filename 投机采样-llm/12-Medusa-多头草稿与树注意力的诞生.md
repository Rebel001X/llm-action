...
四行全部落在两位小数四舍五入的误差内，**恒等式成立、表格自洽**。这张核算表的用处是：**overhead 是 1.18–1.27，不是 1.0**。谁把 "acceleration rate 3.47" 直接当成"快 3.47 倍"，就凭空多算了 22%。
...
4. Medusa 论文报 Vicuna-7B 的 acceleration rate 是 3.47。能不能说"Medusa 让 Vicuna-7B 快了 3.47 倍"？能不能拿它和 EAGLE-3 的接受长度并排比较？
