...
上表里三家都"没有 Medusa"，但**三种没有是不同性质的**，混为一谈会踩不同的坑：

| 性质 | 引擎 | 证据 | 会踩的坑 |
|---|---|---|---|
| **从未实现** | SGLang | 全仓库 grep `medusa` 命中 **0** | 去翻 issue 找"为什么删了" —— 没有删，从来就没有过 |
| **架空**（代码在、路走不通） | vLLM V1 | `medusa.py` 在、`SpeculativeMethod` 枚举在，但 `acceptance_method` / `posterior_threshold` / `posterior_alpha` 三个字段被删 | 看到文件在就以为能复现论文口径 —— 复现不了 |
| **明确移除**（且留下腐烂文档） | TensorRT-LLM 1.2+ | PyTorch backend 下 `MedusaDecodingConfig` 直接抛错；legacy 文档页仍在描述它，链向的 `examples/medusa/README.md` 等路径**全是 404** | 搜到官网文档就以为支持 —— 那页已经腐烂 |

**判据："能 import"不等于"能跑"，"能跑"不等于"能按论文口径跑"。**
Medusa 在 vLLM 里恰好卡在中间那格 —— 代码能 import、权重能加载、也确实能出 token，
但论文里决定加速比的那个判据（typical acceptance）已经没有开关了。
你复现出来的会是一个跑在**拒绝采样**上的 Medusa，那是 **Medusa-1 的 L1 口径**，
不是论文表里 **2.83×** 所对应的 L3 口径（batch=1、MT-Bench、HF 基线，见 §2.3）。
**同一份代码，判据换了，数字就不可比** —— 这正是 [[07-无损的三种口径-分布无损不等于结果相同]] 要分 L1/L2/L3 的原因。

还有一条更隐蔽的信号误读：

> ⚠️ **官方还在发权重 ≠ 引擎还能跑。** NVIDIA 至今在 HF 上挂着 `nvidia/Llama-3.1-8B-Medusa-FP8`，
> 而 TensorRT-LLM 1.2+ 已经跑不了 Medusa —— 那份权重现在只对 ModelOpt 或别的框架有意义。
> **用"权重仓库里还有没有"来判断"这个方法还活着吗"，是不可靠的。** 要看引擎的**枚举 + 文档 + 是否抛错**三样。
