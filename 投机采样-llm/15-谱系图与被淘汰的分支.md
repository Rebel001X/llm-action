...
timeline
    ...
    section 2024 下半年到 2025
        ...
        2024-06 : SmartSpec 2406.14066 Liu Berkeley 验证调度 Goodput
    ...
    section 2026 并行草稿与验证调度
        ...
        2026-02 : P-EAGLE 2602.01469 Hui Amazon NVIDIA 并行草稿
...
**这就是"$\tau$ 高不等于快"的算术根源**：$\tau$ 只是分子，而树把分母乘了 5.5 倍。
...
- **死在什么时候**：**2023-05 就已经写在自己的表里**（Santilli Table 1：纯 Jacobi 在 CPU 上 0.73–0.75×、GPU 上 0.88×，**全线减速**），**2024-02** 由 CLLM（arXiv **2403.00835**，Siqi Kou / Lanxiang Hu，ICML 2024）量化钉死。
- **死因（两条结构性）**：
...
| 当年主张 | 出处 | 推翻证据 |
|---|---|---|
| Medusa「2.3–3.6× 加速」 | arXiv abs 页摘要字段 | **论文自相矛盾**：v3 正文渲染的摘要写 "2.3–2.8×"，**Table 1 最大值仅 2.83×**（3.6× 无表格支撑；最接近的是 Vicuna-7B "Extraction" 单类目 3.62×）。且 Spec-Bench 统一条件下缩水到 **1.44–1.80×**。🔴 Medusa 原文**未给出评测 batch size 与评测 GPU** |
...
Medusa 摘要 2.3–3.6× vs Table 1 最高 2.83×；EAGLE-3 Table 5 正文说 RTX3090 而 caption 说 A100；
...
- `_lab/test_tree.py::test_width_is_worthless_without_marginal_coverage` —— 验证 §5.2 与 §6.7 **大树死因**：边际覆盖增益为 0 时，无论预算多大链都赢。这是"固定大树死了、动态裁剪的树活着"的算术依据。
...
- 可复跑：
  - §8 的渐近线表（实跑输出已贴，其中 `EAGLE-3 tree60 = 0.075 < EAGLE-3 chain10 = 0.317 < DFlash block10 = 0.393`，与 DFlash Table 5 的 c=32 实测排名 0.6× < 1.0× < 1.6× 完全一致）
