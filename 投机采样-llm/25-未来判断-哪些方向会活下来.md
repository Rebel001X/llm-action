...
P-EAGLE 把渐近线抬高了 **30%**，这是实打实的轴 A 进步 —— 但 0.493 仍然意味着**在深 compute-bound 区，开投机会让吞吐掉一半**。这个算术推断和 P-EAGLE 自己实测的形状完全对得上：它 vs EAGLE-3 的加速比从 C=1 的 1.55–1.69× 一路衰减到 C=64 的 1.05–1.25×（MT-Bench / HumanEval / SPEED-Bench 三个集）。
...
**J8｜固定形状的大树死了；活下来的是「按预算动态裁剪的树」。 —— 等级 B（有跨来源冲突，见 §7）**

MLSys 2026 报告：树在 bs=1 只有微弱优势，**bs=64 掉到 1× 以下**；Qwen3-8B + EAGLE + tree(k=21) 在 bs=128 是负收益。**但本库的两份调研笔记对这条数打架**（详见 §7 第 4 条），所以我把它降到 B 级，并把主要论据挂在别处：[[16-树形草稿与树注意力-mask构造与验证]] 已经用可执行验证给出结论 —— **树要赢必须同时满足两个条件，缺一不可**，而"固定的大树"两个都不保证。
...
Aurora（**arXiv:2602.06932（2026-02），ICML 2026**，Together AI；作者含 **Tri Dao、Percy Liang**；代码 github.com/togethercomputer/aurora）把在线 speculator 学习重述成异步 RL：draft model = 策略 $\pi$，target + verifier = 环境；被接受与被拒绝的 token 都流进 data buffer；训练服务器在副本上更新后**热插拔**权重回推理服务器，不中断请求。
...
- **效果**：得到的 AR 模型「表现得像扩散式解码器 —— 一次出多个 token，但仍然从左到右」，**最高 4× 以上**。
...
**4. 本库两份调研笔记对 MLSys 2026 的树数据打架 —— 本篇按低的那个采信。** RS-5 按 A 级记录了「Qwen3-8B + EAGLE + tree(k=21)，GSM8K，bs=1 1.65× → bs=128 < 1×」与「树在 bs=64 掉到 1× 以下」；但 RS-4 对同一篇（arXiv:2601.11580（2026-01））做定向抽取时，结果称**论文正文明确写 "Across all workloads, every SD variant outperforms the no-SD baseline."、且不含该树敏感性分析**，并把「k=21 跌破 1×」标为【待核验】、明令正文不得引用。**处理：J8 降到 B 级，主要论据改挂 [[16-树形草稿与树注意力-mask构造与验证]] 的可执行判据。** 这条坑本身值得记 —— **同一个项目的网站摘要与论文正文可以互相矛盾。**
...
| **J8** 固定大树死了 | B | 某引擎把固定大树设为默认，并给出 **bs≥64 的正收益实测** | MLSys 2026 那篇的 camera-ready 与其网站摘要是否对齐 |
...
- 负收益的完整判定清单（OWL 的 0.81× 在这里）：[[19-负收益全解-什么时候投机反而更慢]]
...
- **SpecDecode-Bench（MLSys 2026，最佳论文荣誉提名）** — <https://specdecode-bench.github.io/>（**本篇引用最多的反方证据**：验证占 42–95%、oracle 缺口 2.2×→4.9×、请求间方差巨大。但它的网站摘要与论文正文在树数据上互相矛盾，见 §7 第 4 条）
...
| 2602.01469 | 2026-02 | P-EAGLE 论文（Amazon + NVIDIA） | §3.2、J3 |
| 2602.06036 | 2026-02 | **DFlash**（Jian Chen / Yesheng Liang / Zhijian Liu，z-lab；**自称 L1，未独立复核**） | J3、§5.2、§6.1 |
| 2602.06932 | 2026-02 | **Aurora**（Together AI，含 Tri Dao / Percy Liang，ICML 2026） | J9 |
...
| 2604.09562 | 2026-04 | StreamServe / SpecuStream（**11–18× 量级异常，未经独立复现**） | O3 |
| 2604.09603 | 2026-04 | ECHO（ICML 2026，已进 SGLang） | J4、J8 |
...
| 2601.11580 | 2026-01 | SpecDecode-Bench 论文（**与其网站摘要在树数据上矛盾**） | §7 第 4 条 |
