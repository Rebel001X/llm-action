...
- **本库依据**：[[16-树形草稿与树注意力-mask构造与验证]] §5.3；测试 `_lab/test_tree.py::test_width_is_worthless_without_marginal_coverage`、`::test_chain_beats_tree_at_small_budget`、`::test_tree_beats_chain_at_large_budget`、`::test_coverage_increases_with_k`。复跑：`python _lab/tree.py --shape`。
...
- `_lab/test_tree.py::test_width_is_worthless_without_marginal_coverage` —— 误解 13：边际覆盖率增益为 0 时，任何预算下链都不输。
