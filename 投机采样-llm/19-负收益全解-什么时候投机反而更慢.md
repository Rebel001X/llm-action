...
> **一条元规则**：投机解码的收益**不是模型属性**，是「模型 × 硬件 × 负载 × 任务」四元组的属性。同一个方法（EAGLE 系）在公开来源里的实测值域是 **0.71× ~ 4.40×** —— 两端的口径分别是：**0.71×** 出自 EAGLE-3 论文 Table 5 的 **batch size = 56**（§5.1 的批大小扫描表），**4.40×** 出自 EAGLE-3 论文 Table 1 的 Llama-3.1-8B、**batch size = 1**、RTX 3090、MT-bench、$T=0$（§7.4）。**这两个数之间隔着的东西，全部是负载而不是方法。** **任何不带这四项的加速比宣称，都应当按营销文案处理。**
...
口径：H100、vLLM v0.10.1.1、每步提议 3 个草稿 token（**链式**）。来源：SpecDecode-Bench，https://arxiv.org/abs/2601.11580 （v1 2025-12-31 / v2 2026-03-18）
...
**口径**：单张 RTX A6000 48GB；16-core Xeon Silver 4309Y；float16；**batch size=1**；目标 = DeepSeek-R1-Distill-Llama-8B（DSL-8B）与 Qwen3-8B（QW3-8B）；共 9 种方法对比。来源：*Scaling Up, Speeding Up*，https://arxiv.org/html/2509.04474v1 （2025-08（v1））
...
一条可直接计算的方法选择判据（本库找到的唯一一条）：**当 prompt-output 重叠的 BLEU-4 超过约 0.6 时，n-gram 稳定优于 EAGLE / EAGLE-3。** 口径：H100、vLLM v0.10.1.1、每步 3 个草稿 token（**链式**）。来源：SpecDecode-Bench，https://arxiv.org/abs/2601.11580
...
Snowflake Arctic 的吞吐表（8×H100，Llama 3.1-70B）在 SWE-Bench 上 suffix decoding 达 286 tok/s vs 无投机 75.8 tok/s，**但主 benchmark 是 max_concurrency = 1 下测的**，且其插件自带 `disable_by_batch_size: 64` —— **他们自己清楚大批下要关。【口径不全，不可当作服务化收益】**（https://www.snowflake.com/en/engineering-blog/fast-speculative-decoding-vllm-arctic/ ）同类：SmartSpec（**注意改名**：该文 v3（2025-07-27）起改名为 **TurboSpec**，今天按 SmartSpec 去 arXiv 搜会搜到新名；本库沿用发表时的原名并引 v2 URL） 的 *"reduces average request latency by up to 3.2×"*，**原文未指明这个 3.2× 出现在哪个模型对 / 哪个 QPS / 哪个数据集，【口径不全，不可引用为代表性数字】**（https://arxiv.org/html/2406.14066v2 ，2024-06）。**处理：任何投机解码加速比，第一件事是找 batch size；找不到就默认它是 1**，并按 §5.1 的衰减曲线打个对折以上的折扣。
...
| SpecDecode-Bench（**链式 k=3**） | 到 batch size 128 仍 > 1.0× | H100 | 多个 | 完整 |
| SpecDecode-Bench（**树 k=21**，v2 新增） | **batch size 64 跌破 1.0×** | H100 | Qwen3-8B / Llama3-70B | 完整（引擎是 **SGLang v0.5.9**） |
...
> **⚠️ 上表第 6、7 行同出一篇，却给出方向相反的交叉点 —— 这不是矛盾，是口径不同，而且它本身就是本篇最好的教材。**
> 这两行原本只有一行。**2026-08-22 复核**（详见 §12.6 第 2 条）确认：*"every SD variant outperforms the no-SD baseline"* 这句在**引言**里，说的是 **vLLM 上五种变体走链式 k=3** 的主实验；而**树式验证是另一套实验、跑在 SGLang v0.5.9 上**，在那套里 **k=21 的树到 batch size 64 就在两个模型的全部负载上跌破 1×**（原文：*"whereas the chain remains above 1× throughout"*）。
> **可判定的读法**：**"这篇论文说投机在 bs=128 仍然赚"是一句缺口径的话。** 正确说法是"**链式**提案到 bs=128 仍赚，**宽树**提案 bs=64 就亏"。同一篇论文、同一台 H100、同样的模型，**只把提案形状从链换成宽树，交叉点就从 >128 提前到 64** —— 交叉点对**提案形状**的敏感度，和它对硬件、batch 的敏感度是同一量级的。谁把这两行合并成一句"SpecDecode-Bench 说 128 以内都赚"，谁就正好踩进 §12.5 这一节要禁的那件事。

### 12.6 两条曾被本库标记为「待核验」的高流传数字（**2026-08-22 已全部核实**），以及一批无出处数字

> **这一节的两条原为【待核验】、且明令"不得作为证据引用"。2026-08-22 各自复核一次，结论是：两条都是真的，但真的方式不一样 —— 一条是抽取工具截断，一条是我读错了版本。原文保留在下面不删，复核结论跟在后面。**

1. **~~【待核验】~~【已核验，2026-08-22】DeepSeek-V3 MTP 的"第二个 token 接受率 85%–90% / 约 1.8 倍 TPS"。** 这两个数字在中英文二手资料中被极其广泛引用，归因于 DeepSeek-V3 技术报告（https://arxiv.org/abs/2412.19437 ，2024-12）。本库当初对 arXiv HTML v1/v2/latest 与 ar5iv 镜像做了 **4 次定向抽取，均未能逐字定位**；其中一次明确报告文档在到达该章节前被截断。

   **复核结果：数字是真的。** 把 arXiv HTML **v1**（549 443 B）、**v2**（549 352 B）与**官方 PDF**（53 页）三份全部落盘、本地正则定位，三种渲染逐字一致。**原文位置：§5.4.3 Multi-Token Prediction Evaluation，PDF 第 35 页**，紧接 §6 Conclusion 之前。逐字：*"Based on our evaluation, the acceptance rate of the second token prediction ranges between 85% and 90% across various generation topics, demonstrating consistent reliability. This high acceptance rate enables DeepSeek-V3 to achieve a significantly improved decoding speed, delivering 1.8 times TPS (Tokens Per Second)."*

   **但按铁律二它仍然不能横向比较，缺项要逐条报出来**：**batch size 原文未给出**；**该节未给推理硬件**（§3.4 只说线上部署在 H800 集群、解码最小单元 40 节点 320 GPU、TP4+SP+DP80+EP320 —— **论文没有说 §5.4.3 的 1.8× 是在这套配置下测的，不许替它连线**）；**TPS 未区分 per-user 输出速度还是系统总吞吐**；数据集只写 *"across various generation topics"*；**接受率的定义口径未给**。γ 隐含 =1。→ **本篇的用法**：可以引用它"存在且为真"，**不可**把它和别的加速比合表。→ 原来那句教训"一个数字被引用一万次，不等于有人核对过原文"仍然成立，但要补上后半句：**"我抽不到"也不等于"原文没有"** —— 该换渲染（HTML→PDF）、落盘本地检索，而不是换个摘要模型再问一遍。

2. **~~【待核验】~~【已核验，2026-08-22】SpecDecode-Bench "树验证 k=21（分支因子 4）在 batch size=64 跌破 1× 加速"。** 该说法出现在项目**网站**摘要（https://specdecode-bench.github.io/ ），而本库对**论文正文**定向抽取的结果称论文未包含该树验证敏感性分析、且明确说所有变体都优于基线，两次抽取互相矛盾，**曾按"以论文正文为准"把它禁用**。

   **复核结果：网站是对的，禁用是错的 —— 那不是矛盾，是版本差。** 本库当时四处引用的都是写死版本号的 `…/html/2601.11580v1`，**从未打开过 v2**。**v1（2025-12-31）确实完全不含树实验**（全文 `Tree-Style` 0 次、`k=21` 0 次、`SGLang` 0 次）；**v2（2026-03-18 修订）新增了整节 "Tree-Style Verification" + Figure 2**。逐字：*"By batch size 64, the k=21 tree falls below 1× speedup on all workloads for both models, whereas the chain remains above 1× throughout."*

   **它和 "every SD variant outperforms the no-SD baseline" 也不矛盾**：后者在**引言**里，讲的是 vLLM 上**链式 k=3** 的五种变体主实验；树实验是**另一套、跑在 SGLang v0.5.9 上**的（原文自陈原因：*"the draft-tree path in the vLLM version we evaluated is not yet sufficiently optimized for a fair comparison"*）。**两句话作用域不同，把引言的一句话当成全文范围的断言，是当初判错的真正原因。**

   **全口径（可引用）**：SGLang v0.5.9；Qwen3-8B 与 Llama3-70B；H100 80GB（8B 单卡 / 70B TP=4）；GSM8K / CNN-DailyMail / ShareGPT / InstructCoder 四个非推理负载；**树深固定 3**；chain 基线 k=3，树 k=6（分支因子 2）与 k=21（分支因子 4，≈EAGLE 的树）；chain 用 FlashAttention-3、树用 FlashInfer；测的是**吞吐比**（generated tokens/sec，开 SD vs 不开）；横轴 batch=1/16/64/128。**bs=1**：Qwen3-8B/GSM8K chain **1.65×** → k=6 **1.68×** → k=21 **1.85×**；Llama3-70B/ShareGPT **1.81× → 1.90× → 2.03×**。**机制**：接受长度 2.25→2.51→**2.92**，同时接受率 0.415→0.300→**0.095**（Llama3-70B/ShareGPT 同向：2.29→2.55→2.93 / 0.429→0.310→0.097）——**宽树买到的那点接受长度，是用崩塌的接受率换的**，而验证吃掉 42%–95% 的时间，所以大 batch 下这笔浪费迅速压过收益。⚠️ **原文未指明这组 bs=1 数字属于 EAGLE 还是 EAGLE-3**（Figure 2 图例两族都有），引用时必须带上这句。

   → **新的教训（比原来那条更该记）**：原来我写的是"项目主页的亮点摘要和论文正文经常不是同一套实验"——**在这里这条是错的，而且它把我导向了错误动作：禁用了一条真数据。** 正确的教训是：**网站与论文"打架"时，先查它们是不是同一个版本**。`abs/` 页会明写 *"last revised …(this version, v2)"*；**把版本号焊死进正文 URL（`…v1`），等于让自己永远读不到修订**。并且：**记录"论文里没有 X"这种否定断言时，必须同时记下核的是哪个版本、哪一天核的 —— 否定断言的保质期比肯定断言短得多。**
...
- SpecDecode-Bench，*Speculative Decoding: Performance or Illusion?*（Xiaoxuan Liu、Jiaxiang Yu、Jongseok Park、Ion Stoica、Alvin Cheung，UC Berkeley；MLSys 2026） — https://arxiv.org/abs/2601.11580 （**v1 2025-12-31 / v2 2026-03-18**）、https://specdecode-bench.github.io/ （网站末次更新 2026-04-02）。§4.2 开销分解、§7.4 的 BLEU-4>0.6 判据出自此。**⚠️ 引用本篇务必用 `abs/` 或 v2：树验证一节只在 v2 里有，本库原先写死的 `…/html/…v1` 读不到它**，曾因此把网站摘要误判成与论文冲突并禁用了一条真数据（2026-08-22 已复核更正，见 §12.6 第 2 条）。
- *Scaling Up, Speeding Up* — https://arxiv.org/html/2509.04474v1 （2025-08（v1））。§7.1 的 7.07 / 0.87× 反例。
...
- SmartSpec / **TurboSpec**（v3 起改名）— https://arxiv.org/html/2406.14066v2 （v1 2024-06-20）。goodput 定义（只算被接受的 token + bonus token）值得采纳；**"up to 3.2×"未指明配置，【口径不全】**。
- DeepSeek-V3 技术报告（DeepSeek-AI） — https://arxiv.org/abs/2412.19437 （2024-12-27 v1 / 2025-02-18 v2）。**其"85–90% 接受率 / 1.8× TPS"曾因 4 次抽取未能逐字定位而标【待核验】；2026-08-22 复核已在 §5.4.3（PDF 第 35 页）逐字定位，HTML v1/v2 与官方 PDF 三渲染一致（见 §12.6 第 1 条）。数字为真，但 batch 与推理硬件原文未给出，本篇只引用其存在性，不与其它加速比合表。**
