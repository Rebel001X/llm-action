...
**但它的结局必须写在这里，否则这条线的历史是断的。** 论文自报 **1.5×–2.3×**（口径：**batch size = 1**、A100 80GB、基线是 HuggingFace 的贪心实现、跨数据集范围；**采样模式只有 1.46×–1.60×**，原文逐字 "Using sampling gives **smaller speedups** as the acceptance ratio is lower"；消融最差档不带 prompt 参考时仅 **1.04×**）。而在**唯一的同硬件横向基准 Spec-Bench** 上，它是全表最弱的一个：

| Spec-Bench 设备 / 目标模型 | Lookahead Overall | 同表最强 |
|---|---|---|
| A100 80GB / Vicuna-13B-v1.3 | **1.30×**（#MAT 1.63） | EAGLE-3 **3.02×**（#MAT 5.71） |
| A100 / Vicuna-7B | **1.34×** | SAMD[EAGLE2] **2.73×** |
| A100 / Vicuna-33B | **1.25×** | EAGLE-2 与 SAMD 并列 **2.59×** |
| RTX 3090 / Vicuna-7B | **1.13×** | SAMD[EAGLE2] **2.38×** |

（**引用，非本库实测**。来源 Spec-Bench Leaderboard — https://github.com/hemingkx/Spec-Bench/blob/main/Leaderboard.md ，榜单最近更新 **2025-04-22**，取回日 2026-08-23。统一评测条件：**batch size = 1 / greedy / FP16 / 3 次运行取均值 / 同设备**；`#MAT` = mean accepted tokens。**榜单未给出各方法的 $\gamma$ 或 n-gram 窗口尺寸**，也未给出温度以外的采样参数。榜单自带警告逐字："model speedup rates may differ across various devices."）

**死因两条，都指回本节的 Jacobi**：① **没有学过的草稿** —— n-gram 池全靠 Jacobi 轨迹自己长出来，接受率天然低（A100 / Vicuna-13B 上 `#MAT` 只有 1.63，即平均每轮净赚不到 0.7 个 token）；② **Jacobi 分支的 FLOPs 要照付** —— §4.5 那笔"迭代次数不减、每轮更贵"的账并没有消失，只是被摊进了每一步的 lookahead branch。**换句话说，Lookahead Decoding 没有解决 Jacobi 的病，它只是把 Jacobi 的产物回收成了草稿。**

**生产状态**：TensorRT-LLM 已把它移出当前 features 文档（legacy），并自 1.2 起随 TRT engine 后端一并删除 `LookaheadDecodingConfig`；vLLM 与 SGLang 的投机方法清单里都没有它。⚠️ **同名陷阱**：vLLM 文档里的 "Lookahead Scheduling" 指 KV 块预留调度，与 Lookahead Decoding 是**毫不相干的两件事**。

它的机制、与 PLD / REST 的分工、以及"被生产引擎删除"的一手证据见 [[11-无模型草稿-promptlookup与ngram与检索]]；它在死亡名单上的完整词条见 [[15-谱系图与被淘汰的分支]]。
...
这是本篇要纠正的第一号误记。原文摘要里的措辞是 "**iteration reductions of up to 2x over a baseline greedy decoder with no loss in quality**"（**该 2× 是迭代次数不是 wall-clock，batch size 原文未给出 —— 全文无 "batch" 一词，逐项见坑二**）—— 主语是 **greedy decoder**，口径是 **L2 贪心等价**，而且如 §5 所述，是相对**增广模型自身的 $p_1$**。把它读成"2018 年就有无损投机采样了"错了两层：既错了口径（L2 不是 L1），也错了参照系（增广模型不是原模型）。
...
> ⚠️ **上一条的"遗产"要加一句时效说明，否则会被读成"Jacobi 换个形态就活了"。** Lookahead Decoding 是这条线唯一进过生产级引擎的形态，而它自己也已经出局：Spec-Bench 统一条件（**batch size = 1** / greedy / FP16 / 同设备）下 **1.13×–1.34×，全表最差**；随后 TensorRT-LLM 把它移入 legacy 并删除了配置类，vLLM / SGLang 从未支持。完整数字与口径见本篇 §3.3，死亡名单词条见 [[15-谱系图与被淘汰的分支]]。**真正活下来的不是 Jacobi 轨迹，而是"从生成历史里攒候选"这个更一般的想法** —— 它后来被后缀树 / 后缀自动机一代接走（[[11-无模型草稿-promptlookup与ngram与检索]]）。
