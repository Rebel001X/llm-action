...
- **数字（完整口径）**：target `nvidia/Kimi-K2.6-NVFP4` + draft `lightseekorg/kimi-k2.6-eagle3.1-mla`；vLLM，**TP=4，GB200**，non-disaggregated；SPEED-Bench coding；`num_speculative_tokens: 3`；指标是 **per-user output throughput（TPS）加速比**。**并发（concurrency）C=1** 时 **2.03×**，**并发 C=4** 时 **1.71×**，**并发 C=16** 时 **1.66×** —— C 就是这张图的负载轴，本篇引用这三个数时必须连 C 一起带上。
...
- **一处必须点名的问题**：**blog 全文未声明任何遗留局限** —— 按本库铁律三，这是一份不完整的发布说明。它给的「长上下文接受长度最多 2× 于 EAGLE 3」是**定性表述，未给具体表** —— 而且 **blog 未给出这句话对应的 batch / 并发 C、上下文长度与数据集**，所以这个 2× 除了方向之外不可用（对照上一条：同一篇 blog 里的加速比就把 C 标全了）。
...
| **Aurora**（论文标题实为《When RL Meets Adaptive Speculative Training》，Aurora 是系统名）| **arXiv:2602.06932（2026-02-06）**，会议**未查证**；<https://www.together.ai/blog/aurora>；github.com/togethercomputer/aurora | Junxiong Wang, Fengxiang Bie, ... **Tri Dao**, **Percy Liang**, ...（机构**未查证**：论文与博客均未列作者机构，仅博客发布于 Together AI 站点）| 见下 | **【有开源实现】** 基于 SGLang，未进 SGLang 主干 |
