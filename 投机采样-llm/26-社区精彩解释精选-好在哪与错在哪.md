...
| **E4 草稿越小越好** | **没有一份材料明说这句话，但也几乎没人正面反驳。** 它是一条"社区默认假设"而非成文错误。最硬的反证有两处：Leviathan §4.1 的 small/base/large 三行表（$\alpha$ 升、加速比降）与 NVIDIA 405B 的 1B/3B/8B **倒 U 形**（3.33× / **3.61×** / 3.04×；口径见 §4.5「另外三份可引数字的材料」：目标 Llama 3.1 405B、4×H200、FP8、`max_batch_size=32`（**这是上限不是实际并发**，全文无并发扫描）、指标是**单请求 output tokens/s 含 TTFT**、`max_draft_len` 未给 —— 这三个数只能内部横比，不许与任何标了并发的吞吐数字比）。**判据是 $\alpha>c$ 的联合权衡，只谈 $\alpha$ 的建议都不完整。**（[[05-接受率alpha-定义口径与怎么测]] §8、[[21-草稿模型怎么训-对齐与在线蒸馏]]） |
