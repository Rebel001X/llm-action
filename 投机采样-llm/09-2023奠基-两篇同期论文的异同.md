...
- **"逐 token 独立验证是不是最优的"是一个两篇都没问过的独立问题。** 两篇都把"每个位置各自判一次、第一次拒绝就截断"当成不言自明的做法，两篇证的也都只是"这样判不改分布"。**这个做法在 2024 年被 Leviathan 本人参与的工作证明是次优的**（Block Verification，**2024-03**，arXiv:2403.10444）。注意它推翻的是**最优性**不是**正确性** —— 逐项展开见 §12.4。
...
| **逐 token 独立验证**（两篇都这么做，但**都没声称、也没证明过**它最优） | Leviathan Algorithm 1 / Chen Algorithm 2 | **被证明是次优的**（正确性不受影响） | **Block Verification**，2024-03，arXiv:2403.10444，Ziteng Sun 等（**Google Research**，作者含 **Yaniv Leviathan 本人**）：整块联合验证，在"每轮期望产出 token 数"这个目标上**最优且永不更差**。逐项见 §12.4 |
...
### 12.4 ⭐ 唯一一条动到算法本身的推翻：逐 token 验证是次优的

上面那句话要加一条精确的补注：**被推翻的并非全是"外围的工程假设"** —— 有一条动的是验证算法本身。它之所以不与上面那句冲突，是因为它动的是**最优性**，不是**正确性**。

| 项 | 值 |
|---|---|
| **标题** | *Block Verification Accelerates Speculative Decoding* |
| **年月 / arXiv** | **2024-03**（v1 **2024-03-15**；v2 2024-07-25）· **arXiv:2403.10444** |
| **作者 / 机构** | Ziteng Sun、Uri Mendlovic、**Yaniv Leviathan**、Asaf Aharoni、Ahmad Beirami、Jae Hun Ro、Ananda Theertha Suresh，**全体署 Google Research**（论文脚注逐字 "Listed in alphabetical order"，通讯邮箱统一 `{zitengsun, urimend, leviathan, …}@google.com`）。**2211.17192 的一作在这份名单里。** |
| **发表去向** | arXiv 上**无 Comments 字段、无 journal-ref**，会议归属**未查证** |
| **口径** | **L1 分布无损**（摘要逐字 "maintains the strong **lossless** guarantee of the standard speculative decoding verification algorithm"） |

**它推翻的那句话**（摘要逐字，核对自 `https://arxiv.org/html/2403.10444v2`，核对日 **2026-08-23**）：
...
对照本篇 §3.3：两篇奠基论文的判据都是**逐位置各掷一次骰子、第一次拒绝就截断**（Leviathan 的 `n ← min{i−1 | r_i > p_i/q_i}`、Chen 的逐位 `if r < min(1, q/p)`）。**两篇都没有把"逐位置判"当成一个需要论证的选择** —— 它们证的是"这样判不改分布"，而"这样判每轮产出最多"是**另一个目标函数下的另一个命题**，谁也没证过。

**必须把这两件事分开说，否则会传成"奠基论文的定理错了"**：

| 命题 | 状态 |
|---|---|
| 逐 token 判据**保持目标模型分布**（**L1** 口径） | **没被推翻**。Block Verification 自己也明确保留这条保证 |
| 逐 token 判据在**"每轮期望产出 token 数"上最优** | **被证伪**。从来没人证过它，而它是假的 |

**5%–8% 那个数字的完整口径（铁律二，逐项）**：

| 铁律二的七项 | Block Verification |
|---|---|
| **1. batch size** | **原文未给出** —— 全文检索 `batch` **0 次** |
| **2. $\gamma$** | **8**（附录另给 $\gamma=4$、$\gamma=6$ 的完整表） |
| **3. 接受长度 / $\alpha$** | 报的是 **block efficiency**（每次目标模型串行调用平均解出的 token 数），$\gamma=8$ / PALM-2-XXS 下 TokenV→BlockV 例如 XSum **3.49 → 3.76**、GSM8K **3.81 → 4.15**。**$\alpha$ 本身未给出** |
| **4. draft–target 组合** | **PALM-2-XXS → PALM-2-S**（另测更小的 PALM-2-XXXS）。**各自参数量原文未给出**，只给了序关系 XXXS ≪ XXS ≪ S |
| **5. 硬件与精度** | **均原文未给出** —— 全文检索 `TPU` / `GPU` 各 **0 次**，无权重精度说明 |
| **6. 测的是什么** | **wall clock time**（延迟侧），外加理想化的 block efficiency。**吞吐原文未给出**。⚠️ **基线是"标准 token 验证的投机解码"，不是不开投机** —— 把它读成"比不投机快 5%–8%"是错的 |
| **7. 任务分布** | LM1B / GPT Prompt（LearnGPT）/ WebQA / PIQA / ShareGPT / XSum / GSM8K / WMT DeEn；各取前 **1000** 条 prompt，输入上限 **512**、输出上限 **128** token，**3 个随机种子取均值** |

数字本体（$\gamma=8$、PALM-2-XXS 起草、batch size 未给出）：**block efficiency 提升 7.00%–10.06%（均值 8.30%）；wall clock 提升 5.36%–8.14%（均值 6.49%）** —— 摘要里那个 "5%-8%" 就是后面这一档。

> ⚠️ **原文的一处措辞错误（本库核对时发现，记在这里以免转述时被带偏）**：§6 讲 wall clock 的那一段写作 "For $\gamma=8$ with PALM-2-XXS as the drafter, **the improvement in block efficiency** ranges from 5.36% to 8.14%"，但同段开头明说这一段测的是 wall clock time，且**上一段已经把 block efficiency 报成 7.00%–10.06%**。该处的 "block efficiency" 应作 "wall clock speedup"。

**为什么这条该写进本篇**：它精确地补上了 §4.1 指出的那个缺口。两篇奠基论文的正确性定理**都是单 token 的**，"整块该怎么判"是一个**原论文根本没有提出过的问题**；三年后回答它的人里有原作者本人。这也是本篇 §8 那份"空白清单"里唯一一条**由奠基论文作者自己补上**的。

**同一批评的树形版本**：*Traversal Verification for Speculative Tree Decoding*，**2025-05**（v1 **2025-05-18**），**arXiv:2505.12398**，Yepeng Weng（**联想 Lenovo AI Technology Center**）、Qiao Hu（**中科院数学与系统科学研究院 AMSS / NCMIS**）等，**NeurIPS 2025 poster**。摘要逐字："**the probability distribution of a sequence differs from that of individual tokens, leading to suboptimal acceptance length**"；做法是把树的验证从自顶向下改成**叶到根遍历**，并同样证明输出分布与目标模型相同（**L1** 口径）。树形草稿的机制见 [[16-树形草稿与树注意力-mask构造与验证]]。
...
- Ziteng Sun, Uri Mendlovic, Yaniv Leviathan, Asaf Aharoni, Ahmad Beirami, Jae Hun Ro, Ananda Theertha Suresh, *Block Verification Accelerates Speculative Decoding* — https://arxiv.org/abs/2403.10444 （**2024-03**，v1 2024-03-15，v2 2024-07-25，**Google Research**；发表去向 arXiv 无 Comments 字段，**未查证**）。元数据与摘要经 `export.arxiv.org` Atom API + `https://arxiv.org/html/2403.10444v2` 逐字核对（核对日 **2026-08-23**）。**它讲得好在**：把"逐 token 验证是次优的"从经验观察证成**定理**（"optimal in the expected number of tokens produced each iteration"、"never worse"），而且**由奠基论文的一作本人合作完成** —— 本库找到的唯一一条奠基论文作者对自己算法的正式修正（§12.4）。**要指出的不严谨之处有三处**：① 全文**没有 batch size、没有硬件型号、没有权重精度**（检索 `batch` / `TPU` / `GPU` 各 0 次），5%–8% 因此**不可与任何别家的加速比横向比较**；② 基线是"标准 token 验证的投机解码"而非不开投机，转述时极易被读成"再快 5%–8% 的绝对加速"；③ §6 讲 wall clock 的那段把 "wall clock speedup" 写成了 "block efficiency"，与上一段的数字撞车（详见 §12.4 的告示）。
- Yepeng Weng, Qiao Hu, Xujie Chen, Li Liu, Dianwen Mei, Huishi Qiu 等, *Traversal Verification for Speculative Tree Decoding* — https://arxiv.org/abs/2505.12398 （**2025-05**，v1 2025-05-18，**联想 Lenovo AI Technology Center** + **中科院 AMSS / NCMIS**，**NeurIPS 2025 poster**；会议信息取自 arXiv Comments 字段）。**它讲得好在**：把上一条的批评推广到树形草稿，并点破了同一个根因 —— "the probability distribution of a sequence differs from that of individual tokens"。**要指出的是**：本篇只用了它的问题陈述，**其加速数字的口径（batch size / 硬件 / 模型对）本篇未提取，标「未查证」**，不要引用。
