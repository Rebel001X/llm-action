...
1. **输出与输入没有字面重复时，$\alpha\to0$，收益归零。**
   ...
   同源实测（H100，vLLM v0.10.1.1，每步提议 3 个草稿 token，单位=平均接受 token 数）：n-gram 在 InstructCoder 上 7.27，在 GSM8K 上 **1.41** —— **同一方法，任务一换差 5.2 倍。**
...
- **SpecDecode-Bench** — <https://arxiv.org/html/2601.11580v1> ；项目页 <https://specdecode-bench.github.io/>。BLEU-4 ≈ 0.6 的方法选择判据、n-gram 的方差（InstructCoder 7.27 std 4.19 vs GSM8K 1.41 std 0.45）、草稿开销 <2% 与显存 0 均出自此文。⚠️ 该项目**网站**摘要里"树验证 k=21 在 batch size 64 跌破 1×"的说法与论文正文冲突，本篇**未引用**该条。
