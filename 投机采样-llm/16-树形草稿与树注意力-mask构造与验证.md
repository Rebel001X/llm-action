...
树注意力**不是新算子**，它就是一张换了形状的 mask：把树拍平成一维序列，让每个节点只能看见自己的**祖先链**，这一次前向的结果与"把每条根到叶路径分别当成普通因果序列各跑一次"**逐元素相等**（本库实测误差 $4.4\times10^{-16}$）。真正难的不是实现，是回答另一个问题：**同样的验证预算，该排成深链还是宽树？** 本库初稿在这里给过一个错答案（「树要赢必须同时满足两个条件」），2026-08-22 经对抗审稿推翻并改正：**树的渐近天花板恒高于链**，所以问题从来不是「树赢不赢」，而是「**预算够不够大到让它兑现**」。详见 §5.3 与文末勘误。
...
**它的危险在于不会报错。** 形状对、mask 对、程序跑通，只是目标模型算出的 $p_i$ 对应的不再是那条真实前缀 —— 于是 [[04-拒绝采样修正-无损性的完整证明]] §8 第 1 条的对齐前提被破坏，**L1 分布无损悄悄失效**（$T=0$ 下 L2 贪心等价同样失效），表现只是"接受率莫名偏低"。很多人会去调草稿模型，其实该去查 position id。
...
**草稿一致度 agree=0.95**，覆盖率 $c_1{=}0.6081,\ c_2{=}0.7601,\ c_4{=}0.8657,\ c_8{=}0.9193$

| 预算 $n$ | 链 ($k{=}1$) | 2 叉树 | 4 叉树 | 8 叉树 |
|---|---|---|---|---|
| 8 | **1.5227** | 1.3380 | 0.8657 | 0.9193 |
| 16 | 1.5512 | **1.7772** | 0.8657 | 0.9193 |
| 64 | 1.5517 | **2.3648** | 1.6152 | 0.9193 |
| 128 | 1.5517 | **2.5577** | 2.2640 | 1.7645 |
| 1024 | 1.5517 | **2.9006** | 2.8257 | 2.5416 |

**草稿一致度 agree=0.80**，覆盖率 $c_1{=}0.6081,\ c_2{=}0.6081,\ c_4{=}0.8277,\ c_8{=}0.8900$

| 预算 $n$ | 链 | 2 叉树 | 4 叉树 | 8 叉树 |
|---|---|---|---|---|
| 8 | **1.5227** | 0.9779 | 0.8277 | 0.8900 |
| 64 | **1.5517** | 1.4227 | 1.5128 | 0.8900 |
| **128** | 1.5517 | 1.4733 | **2.0799** | 1.6822 |
| 1024 | 1.5517 | 1.5341 | **2.5492** | 2.3873 |

**草稿一致度 agree=0.50**，覆盖率 $c_1{=}0.6081,\ c_2{=}0.6081,\ c_4{=}0.7601,\ c_8{=}0.8657$

| 预算 $n$ | 链 | 2 叉树 | 4 叉树 | 8 叉树 |
|---|---|---|---|---|
| 8 | **1.5227** | 0.9779 | 0.7601 | 0.8657 |
| 64 | **1.5517** | 1.4227 | 1.3380 | 0.8657 |
| **128** | 1.5517 | 1.4733 | **1.7772** | 1.6152 |
| 1024 | 1.5517 | 1.5341 | 2.1111 | **2.2640** |

> **初稿只扫到 $n=64$ 就下了结论，而反超恰好发生在下一格（$n=128$）。**
> 这三张表的最后两行是 2026-08-22 补的，它们直接推翻了初稿 §5.3 的第二条结论。

### 5.3 结论（2026-08-22 重写：初稿这一节有两处错）

**流行说法一："树总是比链好，因为验证同价。"** 这条**仍然是错的**：agree=0.95、预算 8 时，链 1.5227 就是最优，二叉树只有 1.3380。**预算太小时，加宽度等于砍深度，砍得不值。**

**流行说法二："草稿不准就该把树加宽。"** 初稿说这条"也错，而且错得更彻底"，理由是"agree=0.80 时 $c_2=c_1$，边际贡献恰好是 0，于是无论预算多大链都赢"。**这个理由和这个结论都是错的**，两处都要更正：

**更正一：$c_2=c_1$ 是显示精度造成的假象。** top-$k$ 覆盖率对 $k$ **严格递增** —— top-$k$ 候选集包含 top-1 再加 $k-1$ 个 token，而 $p$ 是全支撑的，多加一个 token 必然多加一份正概率。表里那个"$0.6081$ 对 $0.6081$"真实增益约 $10^{-5}$，不是 0。实测 40 种子 × 2 一致度 × 3 个 $k$ 共 240 组，$c_k\le c_1$ 的样本数是 **0**（`_lab/test_tree.py::test_coverage_gain_is_always_strictly_positive`）。

**更正二："无论预算多大链都赢"是把表停早了。** agree=0.80 时 $c_4=0.8277$ 比 $c_1$ 高 36%，**预算 128 时 4 叉树 2.0799 反超链 1.5517（+34%）**；agree=0.50 时同样在 128 反超。初稿的表只扫到 64，**恰好停在反超前一格**。

于是正确的原理是：

> **每种形状都有一个与预算无关的渐近天花板** $\displaystyle\sum_{d\ge1}c_k^{\,d}=\frac{c_k}{1-c_k}$。
> 因为 $c_k>c_1$ 恒成立、而 $x\mapsto x/(1-x)$ 严格增，**树的天花板恒高于链**：
>
> | | 链 | 2 叉 | 4 叉 | 8 叉 |
> |---|---|---|---|---|
> | agree=0.95 | 1.55 | 3.17 | 6.45 | 11.40 |
> | agree=0.80 | 1.55 | 1.55 | **4.80** | 8.09 |
> | agree=0.50 | 1.55 | 1.55 | **3.17** | 6.45 |
>
> 所以问题从来不是「树赢不赢」，而是「**预算够不够大到让它兑现**」。

**真正的判据是「增益多大 vs 预算多大」**：

| 增益 $c_k/c_1$ | 兑现所需预算 | 例 |
|---|---|---|
| +36%（agree=0.80, $k{=}4$） | **128** | 表中 2.0799 vs 1.5517 |
| +25%（agree=0.95, $k{=}2$） | **16** | 1.7772 vs 1.5512 |
| $\sim10^{-5}$（agree=0.80, $k{=}2$） | $\gtrsim2^{31}$，**工程上等于永不** | 预算 65 536 仍不反超 |

（`_lab/test_tree.py::test_higher_coverage_always_means_higher_ceiling`、`::test_tree_actually_overtakes_when_gain_is_meaningful`、`::test_budget_needed_grows_as_the_gain_shrinks`）

**"迟早会赢"在工程上不等于"会赢"** —— 这是本节最该带走的一句话。天花板更高是数学事实，但如果兑现它需要 $2^{31}$ 个节点的验证预算，那条天花板对你没有任何意义。

### 5.4 由此推出的工程结论

1. **最优形状不是常数。** 它随预算变（表的行 —— 而且 §5.3 说明**扫描区间不够宽就会得出相反结论**）、随草稿在当前位置的置信度变（表的组）。**静态树一定在某些位置上是错的** —— 这正是 EAGLE-2 之后转向"按草稿置信度动态决定树形状"的原因（[[13-EAGLE三代-特征级自回归的演进]]）。
...
2. **草稿整体跑偏（边际覆盖率增益极小）时**：§5.3，天花板虽仍更高，但兑现所需预算大到不现实。此时该做的是**换/重训草稿**，不是加宽树。
...
1. 为什么兄弟节点之间必须互相不可见？如果让它们互相可见会发生什么？
   <details><summary>答案要点</summary>兄弟是互斥猜测，代表"如果这一步选 A"与"如果这一步选 B"两条不同的未来。互相可见等于让目标模型在一个**不存在的混合前缀**上算概率，$p_i$ 不再对应任何一条真实路径，[[04-拒绝采样修正-无损性的完整证明]] 的对齐前提被破坏，**L1 分布无损**失效。</details>
...
- `_lab/test_tree.py::test_coverage_gain_is_always_strictly_positive` —— **§5.3 更正一**：240 组样本里零增益出现 0 次，$c_k>c_1$ 恒成立。
- `_lab/test_tree.py::test_higher_coverage_always_means_higher_ceiling` —— 天花板序关系恒真（360 组全过）。
- `_lab/test_tree.py::test_tree_actually_overtakes_when_gain_is_meaningful` —— 增益 >1% 时预算 4096 下 306/306 反超。
- `_lab/test_tree.py::test_budget_needed_grows_as_the_gain_shrinks` —— 判据是「增益 vs 预算」：+36% 需 128，$10^{-5}$ 需 $2^{31}$。
  （**2026-08-22 更正**：初稿说的"增益为 0"其实是显示精度造成的假象，真实增益约 $10^{-5}$；$c_k>c_1$ 恒成立，正确判据是"增益多大 vs 预算多大"。详见第 16 篇 §5.3 与其勘误。）
- `_lab/test_tree.py::test_original_never_wins_claim_is_false_at_larger_budget` —— **把被推翻的反例钉住**：预算 128 时 4 叉树 +34%。
- 可复跑：
...
初稿 §5.3 有**两处错**，由对抗审稿查出，均已在正文原地更正并保留痕迹：

1. **把"$c_2$ 与 $c_1$ 显示成同一个 0.608"当成了"边际增益恰好是 0"。** 实际增益约 $10^{-5}$；数学上 $c_k>c_1$ 恒成立（top-$k$ 集合更大而 $p$ 全支撑），"零增益"不可能发生。
2. **由此得出的"无论预算多大链都赢"是把扫描区间停早了。** 表只到 $n=64$，而反超发生在 $n=128$（4 叉树 2.0799 vs 链 1.5517）。

**还有第三个问题在测试侧**：原来那条 `test_budget_needed_grows_as_the_gain_shrinks` 是**单种子命题** —— 200 个种子里只有 87 个（43.5%）成立，`seed=0` 恰好是少数派。根因是 `make_draft` 在 agree=0.5/0.8 上落到并列点，$q$ 的 top-1/top-2 由浮点噪声决定。

**这正是本库自己列为踩过的坑（"统计命题只采一个样本"）**，却在这一节又踩了一次。修正后的五条测试全部跑 40 个种子。

**教训**：`verify.py --tests` 只能保证"引用的测试存在"，**保证不了"测试真的验证了正文的说法"，更保证不了"扫描区间足够宽"**。这两件事目前只能靠人或对抗审稿。
