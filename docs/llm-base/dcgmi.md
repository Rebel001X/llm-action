...
> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[ai-infra/算力/GPU工作原理]] [[ai-infra/ai-hardware/硬件对比]] [[llm-eval/llm-performance/大模型场景下训练和推理性能指标名词解释]]
> 📖 导读：本文讲 DCGMI（NVIDIA Data Center GPU Manager 命令行工具）的用法——GPU 拓扑/统计/诊断/策略/健康监控/配置管理等子命令；适合在数据中心运维、排查 GPU 故障、监控训练推理集群算力时查阅。
