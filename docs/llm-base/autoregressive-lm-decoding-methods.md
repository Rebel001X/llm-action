> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[llm-inference/解码策略]] [[llm-algo/transformer/模型架构]] [[llm-inference/README]] [[llm-algo/旋转编码RoPE]]
> 📖 导读：系统梳理自回归语言模型的解码与采样方法（Greedy / Beam Search / Top-k / Top-p / Temperature 等），并结合 HuggingFace `generate` 源码逐参数解析；适合做文本生成调参、理解推理阶段采样策略时查阅。
