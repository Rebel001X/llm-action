...
> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[ai-framework/megatron-lm/README]] [[ai-framework/deepspeed/README]] [[ai-infra/网络/集合通信原语]]
> 📖 导读：本文讲 Alpa 自动并行系统——用动态规划做 inter-op（流水线 stage 切分）、用整数线性规划做 intra-op（算子内张量切分），兼顾两类并行并依赖 JAX/XLA 的 GSPMD；适合研究大模型自动分布式策略寻优时阅读。
