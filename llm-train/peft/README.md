...
# PEFT 高效参数微调实操
> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[ai-framework/huggingface-peft/README]] [[llm-train/peft/PEFT-API]] [[llm-train/peft/Prompt-Tuning]] [[llm-train/peft/Prefix-Tuning]]
> 📖 本文导读：本文是 PEFT(Parameter-Efficient Fine-Tuning)高效参数微调的总览与实操，覆盖环境搭建、Jupyter 配置、DeepSpeed 与各类微调方法的端到端流程；适合动手跑通 LoRA/Prompt/Prefix 等 PEFT 微调时对照查阅。
