# BLOOM 训练背后的技术：Megatron-DeepSpeed 3D 并行实战
> 📍 导航：[[00-知识地图]]
...
> 📖 本文导读：复盘 176B 参数 BLOOM 大模型的训练工程实践，详解如何用 Megatron-DeepSpeed 把 ZeRO 数据并行、张量并行、流水线并行组合成 3D 并行，并涵盖 BF16 优化器、融合 CUDA 核函数等关键组件；适合想了解超大规模模型训练全栈方案与并行策略落地细节时阅读。
