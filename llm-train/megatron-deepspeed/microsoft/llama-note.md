...
# Megatron-DeepSpeed LLaMA 训练实操笔记
> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[llm-train/megatron-deepspeed/README]] [[ai-framework/deepspeed/README]] [[ai-framework/megatron-lm/README]] [[llm-train/megatron/README]]
> 📖 本文导读：基于 NVIDIA PyTorch 容器 + Microsoft Megatron-DeepSpeed，从环境搭建、依赖安装到 `pretrain_llama_distributed.sh`（TP=2/PP=2/ZeRO）多卡预训练 LLaMA 的完整命令与训练日志实录，适合需要照着跑通 Megatron-DeepSpeed LLaMA 分布式预训练、排查环境与并行配置时查阅。
