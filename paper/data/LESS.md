> 📍 导航：[[00-知识地图]]
> 🔗 相关：[[llm-train/peft/PEFT-API]] [[llm-train/README]] [[llm-alignment/RLHF]]
> 📖 导读：本文精读 LESS 论文——用 LoRA 热身 + 低秩梯度相似度搜索，从大规模指令数据中只挑选 5% 最有影响力的数据做目标指令微调，效果常优于全量训练；适合做数据选择/指令微调/高效训练方向时阅读。
